...
# coding: utf-8
# =============================================================================
# OOM Detection Pipeline — XGBoost (multi-class classifier)
#
# 입력 : Cassandra feature_store.oom_b_riclog (적재 시점에 ts_ns 정렬·split 완료)
# 출력 : XGBoost 모델 + cv_results.json + training_history (CSV/JSON) + 평가 메트릭
...
@component(
    ...
    packages_to_install=['xgboost', 'scikit-learn', 'imbalanced-learn'],
)
def train_export_model(
    ...
    # ---- imports (KFP component는 폐쇄 환경 → 함수 안에서 import) ----
    ...
    #   eval_set으로 per-iteration train mlogloss를 캡처한다.
    #========================================================================
    ...
    model = build_model()
    model.fit(X_all_res, y_all_res, eval_set=[(X_all_res, y_all_res)], verbose=False)

    #-- training_history: 최종 재학습의 per-iteration train mlogloss
    er = model.evals_result_  # {'validation_0': {'mlogloss': [...]}}
    train_log = er.get('validation_0', {}).get('mlogloss', [])
    history = {
        'epoch': list(range(1, len(train_log) + 1)),
        'train_mlogloss': [float(x) for x in train_log],
    }
    ...
    print(f"[history] saved {len(history['epoch'])} iterations")
...
@dsl.pipeline(name="oom_xgboost Pipeline", description="OOM detection — XGBoost")
def super_model_pipeline(
    featurepath: str, modelname: str, modelversion: str,
    n_estimators: int = 300, max_depth: int = 6,
    learning_rate: float = 0.1, scale_pos_weight: float = 1.0,
