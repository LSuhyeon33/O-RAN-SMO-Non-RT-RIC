...
# coding: utf-8
# =============================================================================
# OOM Detection Pipeline — Transformer (multi-class classifier, sliding window)
# Encoder-only Transformer + GlobalAveragePooling1D + Dense softmax.
...
def train_export_model(
    ...
    def build_model():
        #-- 모델 — Encoder-only Transformer
        def encoder_block(x, d_model, nhead, ff_dim, dropout):
            attn = MultiHeadAttention(num_heads=nhead, key_dim=d_model // nhead, dropout=dropout)(x, x)
            x1 = LayerNormalization(epsilon=1e-6)(Add()([x, attn]))
            ff = Dense(ff_dim, activation='relu')(x1)
            ff = Dense(d_model)(ff)
            ff = Dropout(dropout)(ff)
            return LayerNormalization(epsilon=1e-6)(Add()([x1, ff]))

        inputs = Input(shape=(int(window_size), N_FEATURES))
        x = Dense(int(d_model))(inputs)  # 입력 피처 수 → d_model 차원으로 사영
        for _ in range(int(num_layers)):
            x = encoder_block(x, int(d_model), int(nhead), int(ff_dim), float(dropout))
    ...
    print(f"  >>> Hyperparameters: d_model={d_model}, nhead={nhead}, num_layers={num_layers}, "
          f"ff_dim={ff_dim}, window_size={window_size}, dropout={dropout}, batch_size={batch_size}")
    print(f"{'#'*60}\n")
    ...
    cv_summary = {
        'hyperparameters': {
            ...
            'ff_dim': ff_dim, 'window_size': window_size, 'dropout': dropout,
...
@dsl.pipeline(name="oom_transformer Pipeline", description="OOM detection — Transformer")
def super_model_pipeline(
    featurepath: str, modelname: str, modelversion: str,
    d_model: int = 128, nhead: int = 8, num_layers: int = 2, ff_dim: int = 256,
    window_size: int = 20, dropout: float = 0.1,
    ...
    op = train_export_model(
        ...
        d_model=d_model, nhead=nhead, num_layers=num_layers, ff_dim=ff_dim,
