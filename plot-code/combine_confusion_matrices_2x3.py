...
사용 예시:
python combine_confusion_matrices_2x3.py \
  ...
  --img6 xgboost.png \
  --output combined_confusion_matrices_2x3.png \
  --dpi 300

기능:
- 6개 이미지를 동일 크기로 맞춘 뒤 2x3으로 배치
...
def combine_images_2x3(
    ...
    positions = [
        ...
        (margin + (tile_w + gap) * 2, margin),             # row 1 col 3
        (margin, margin + tile_h + gap),                   # row 2 col 1
        (margin + tile_w + gap, margin + tile_h + gap),    # row 2 col 2
        (margin + (tile_w + gap) * 2, margin + tile_h + gap),  # row 2 col 3
    ]
...
def main():
    parser = argparse.ArgumentParser(description="Combine 6 images into one 2x3 image.")
    parser.add_argument("--img1", required=True, help="Top-left image path")
    parser.add_argument("--img2", required=True, help="Top-middle image path")
    parser.add_argument("--img3", required=True, help="Top-right image path")
    parser.add_argument("--img4", required=True, help="Bottom-left image path")
    parser.add_argument("--img5", required=True, help="Bottom-middle image path")
    parser.add_argument("--img6", required=True, help="Bottom-right image path")
    parser.add_argument("--output", default="combined_confusion_matrices_2x3.png", help="Output image path")
