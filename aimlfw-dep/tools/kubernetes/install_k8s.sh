...
# run buildkit instance (수정된 섹션)
# 기존에 동일한 이름의 컨테이너가 있다면 삭제 (재설정 반영을 위함)
sudo nerdctl rm -f buildkitd 2>/dev/null || true

# 인증서 마운트 옵션 정의 (공통 사용)
# 호스트의 인증서 저장소 2곳을 컨테이너 내부로 읽기 전용(:ro) 마운트
CERT_MOUNTS="-v /etc/ssl/certs:/etc/ssl/certs:ro -v /usr/share/ca-certificates/:/usr/share/ca-certificates/:ro"

echo "Running buildkit instance with host certificates mounted..."

if is_wsl; then
  # WSL 환경: 네트워크 호스트 모드 + 인증서 마운트 + 자동 재시작 추가
