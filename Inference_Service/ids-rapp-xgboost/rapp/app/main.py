...
def _run_evaluate(batch_size: int, warmup: int):
    ...
    try:
        ...
        for scen in SCENARIO_TO_LABEL.keys():
            ...
            X, y = preprocess.transform_rows_per_scenario(df, scenario=scen)
...
def predict_one():
    try:
        ...
        # 트리 모델은 윈도윙 없이 한 행(20개 피처)이 곧 한 샘플.
        # 'features' 를 받되, 하위호환을 위해 'window' 키도 폴백 허용.
        features = body.get('features', body.get('window'))
        if features is None:
            return jsonify({'error': 'body must contain "features"'}), 400
        X = preprocess.transform_single_row(features)
